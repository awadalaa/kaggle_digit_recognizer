...
class Config(object):
  ...
  img_width = 28
  img_height = 28
  img_total_pixels = img_width * img_height
  hidden_size = 100
  max_epochs = 20
  num_classes = 10
  early_stopping = 2
  dropout = 0.5
  learning_rate = 5e-4
  training_iterations = 10000 # set to 20000 on local environment to get 0.99 accuracy
  iterations_per_run = 100
  batch_size = 100
  validation_size = 2000
  image_to_display = 25

class NN_Model(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    self.labels_placeholder = tf.placeholder(dtype=tf.int32, shape=[None, self.config.num_steps], name='Target')
...
def test_NN():
  ...
  with tf.Session() as session:
    ...
    session.run(init)
    if config.generate_text_only == False:
